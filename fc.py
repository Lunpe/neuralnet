...
class FCNetwork():
	""" A fully-connected neural network.
		Assumes NeLu activation function and softmax as loss function."""
	...
	def train(self, data):
		...
		for epoch in xrange(self.n_epochs):
			...
			for n in xrange(0, len(data[0])-self.batch_size, self.batch_size):
				self.train_batch((data[0][n: n + self.batch_size],
	...
	def train_batch(self, batch):
		...
		for i, (w, b) in enumerate(zip(self.weights, self.biases)[:-1]):
			# Using NeLu activation function
		...
		for l in xrange(1, len(self.weights)+1): # NeLu makes it easy
